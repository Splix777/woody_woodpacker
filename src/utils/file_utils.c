...
static int write_to_file(int fd, void *data, size_t data_size, size_t *offset)
{
    size_t n_bytes;
...
int write_elf(t_woody_context *context)
{
    ...
    if (context->elf.is_64bit)
    {
        ...
        write_to_file(context->file.output_fd, context->elf.elf64.ehdr, sizeof(Elf64_Ehdr), &offset);
        add_zero_padding(context->file.output_fd, context->elf.elf64.ehdr->e_phoff, &offset);

        print_verbose(context, "Writing 64-bit ELF Program Headers\n");
        write_to_file(context->file.output_fd,
                      context->elf.elf64.phdr,
        ...
        for (int i = 0; i < context->elf.elf64.ehdr->e_shnum; i++)
        {
            ...
            {
                add_zero_padding(context->file.output_fd, context->elf.elf64.shdr[i].sh_offset, &offset);
                if (i == context->elf.elf64.payload_section_index && context->elf.elf64.cave)
                    write_to_file(context->file.output_fd,
                                  context->elf.elf64.section_data[i],
                                  context->elf.elf64.shdr[i].sh_size + INJECTION_PAYLOAD_64_SIZE,
                                  &offset);
                else
                    write_to_file(context->file.output_fd,
                                  context->elf.elf64.section_data[i],
        ...
        add_zero_padding(context->file.output_fd, context->elf.elf64.ehdr->e_shoff, &offset);

        print_verbose(context, "Writing 64-bit ELF Section Headers\n");
        for (int i = 0; i < context->elf.elf64.ehdr->e_shnum; i++)
            write_to_file(context->file.output_fd,
                          &context->elf.elf64.shdr[i],
                          sizeof(Elf64_Shdr),
                          &offset);
    }
    else
    {
        ...
        write_to_file(context->file.output_fd, context->elf.elf32.ehdr, sizeof(Elf32_Ehdr), &offset);
        add_zero_padding(context->file.output_fd, context->elf.elf32.ehdr->e_phoff, &offset);

        print_verbose(context, "Writing 32-bit ELF Program Headers\n");
        write_to_file(context->file.output_fd,
                      context->elf.elf32.phdr,
        ...
        for (int i = 0; i < context->elf.elf32.ehdr->e_shnum; i++)
        {
            ...
            {
                add_zero_padding(context->file.output_fd, context->elf.elf32.shdr[i].sh_offset, &offset);
                if (i == context->elf.elf32.payload_section_index && context->elf.elf32.cave)
                    write_to_file(context->file.output_fd,
                                  context->elf.elf32.section_data[i],
                                  context->elf.elf32.shdr[i].sh_size + INJECTION_PAYLOAD_32_SIZE,
                                  &offset);
                else
                    write_to_file(context->file.output_fd,
                                  context->elf.elf32.section_data[i],
        ...
        add_zero_padding(context->file.output_fd, context->elf.elf32.ehdr->e_shoff, &offset);

        print_verbose(context, "Writing 32-bit ELF Section Headers\n");
        for (int i = 0; i < context->elf.elf32.ehdr->e_shnum; i++)
            write_to_file(context->file.output_fd,
                          &context->elf.elf32.shdr[i],
                          sizeof(Elf32_Shdr),
                          &offset);
    }
