...
static int already_signed(t_woody_context* context) {
    ...
    if (e_ident[10] == 0x57 && e_ident[11] == 0x4f && e_ident[12] == 0x4f
        && e_ident[13] == 0x44) {
    ...
    print_verbose(context, "File not signed, found: %c%c%c%c\n", e_ident[10],
        e_ident[11], e_ident[12], e_ident[13]);
...
static int validate_magic(t_woody_context* context) {
    ...
    if (memcmp(e_ident, ELFMAG, SELFMAG) != 0 || e_ident[EI_MAG0] != ELFMAG0
        || e_ident[EI_MAG1] != ELFMAG1 || e_ident[EI_MAG2] != ELFMAG2
        || e_ident[EI_MAG3] != ELFMAG3) {
        print_verbose(context, "%s Invalid Magic: %c%c%c%c\n", BOXED_CROSS,
            e_ident[EI_MAG0], e_ident[EI_MAG1], e_ident[EI_MAG2],
            e_ident[EI_MAG3]);
...
static int validate_class_and_machine(t_woody_context* context) {
    ...
    if ((e_class != ELFCLASS64 && e_class != ELFCLASS32)
        ...
        print_verbose(context, "%s Invalid Class: %d or Machine: %d\n",
            BOXED_CROSS, e_class, machine);
        return ERR_INVALID_ELF;
    }
    context->elf.is_64bit = (e_class == ELFCLASS64);

    print_verbose(context, "ELF 64-bit: %s",
        context->elf.is_64bit ? "True\n" : "False\n");
...
static int validate_type(t_woody_context* context) {
    ...
    print_verbose(context, "Executable Type: %s\n",
...
static int validate_header_fields(t_woody_context* context) {
    if (context->elf.is_64bit) {
        ...
        if (sizeof(Elf64_Ehdr) > context->file.file_size) {
            print_verbose(context, "%s ELF Header Size: %ld, File Size: %ld\n",
...
static int validate_entry_point(t_woody_context* context) {
    if (context->elf.is_64bit) {
        Elf64_Ehdr* ehdr = (Elf64_Ehdr*)context->file.file_buff;
        if (ehdr->e_entry == 0) {
            print_verbose(context, "%s Invalid Entry Point: %ld\n",
        ...
        Elf32_Ehdr* ehdr = (Elf32_Ehdr*)context->file.file_buff;
        if (ehdr->e_entry == 0) {
            print_verbose(context, "%s Invalid Entry Point: %d\n", BOXED_CROSS,
                ehdr->e_entry);
            return ERR_INVALID_ELF;
        }
    }
