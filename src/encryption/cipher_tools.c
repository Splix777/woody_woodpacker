...
int encrypt_text_section(t_woody_context* context) {
    ...
    if (context->elf.is_64bit) {
        ...
        context->elf.elf64.text_size
            = context->elf.elf64.shdr[text_index].sh_size;
        context->elf.elf64.text_entry
            = context->elf.elf64.shdr[text_index].sh_addr;

        if (generate_key(context) != SUCCESS)
            return ERR_ENCRYPTION;

        if (encrypt_64(text_data, context->elf.elf64.text_size,
        ...
        context->elf.elf32.text_size
            = context->elf.elf32.shdr[text_index].sh_size;
        context->elf.elf32.text_entry
            = context->elf.elf32.shdr[text_index].sh_addr;

        if (generate_key(context) != SUCCESS)
            return ERR_ENCRYPTION;

        if (encrypt_32(text_data, context->elf.elf32.text_size,
                context->encryption.key32)
            != 0)
            return ERR_ENCRYPTION;
    }
