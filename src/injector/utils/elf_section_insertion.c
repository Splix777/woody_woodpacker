...
static int update_string_table(t_woody_context* context) {
    ...
    if (context->elf.is_64bit) {
        Elf64_Shdr* shstrtab
            = &context->elf.elf64.shdr[context->elf.elf64.ehdr->e_shstrndx];
        size_t shstrtab_size = shstrtab->sh_size;
        char*  shstrtab_data
            = context->elf.elf64
                  .section_data[context->elf.elf64.ehdr->e_shstrndx];
        ...
        memcpy(updated_shstrtab + shstrtab_size, new_section_name,
            new_section_name_len);

        context->elf.elf64.section_data[context->elf.elf64.ehdr->e_shstrndx]
            = updated_shstrtab;
        context->elf.elf64.shdr[context->elf.elf64.ehdr->e_shstrndx].sh_size
            = new_shstrtab_size;
    } else {
        Elf32_Shdr* shstrtab
            = &context->elf.elf32.shdr[context->elf.elf32.ehdr->e_shstrndx];
        size_t shstrtab_size = shstrtab->sh_size;
        char*  shstrtab_data
            = context->elf.elf32
                  .section_data[context->elf.elf32.ehdr->e_shstrndx];
        ...
        context->elf.elf32.section_data[context->elf.elf32.ehdr->e_shstrndx]
            = updated_shstrtab;
        context->elf.elf32.shdr[context->elf.elf32.ehdr->e_shstrndx].sh_size
            = new_shstrtab_size;
    }
...
static int update_segment_values(t_woody_context* context, int segment_index) {
    if (context->elf.is_64bit) {
        size_t new_segment_size
            = context->elf.elf64.phdr[segment_index].p_memsz
            + INJECTION_PAYLOAD_64_SIZE;
        context->elf.elf64.phdr[segment_index].p_filesz = new_segment_size;
        context->elf.elf64.phdr[segment_index].p_memsz  = new_segment_size;
        set_elf_segment_permission(context, segment_index, PF_W | PF_X | PF_R);
    } else {
        size_t new_segment_size
            = context->elf.elf32.phdr[segment_index].p_memsz
            + INJECTION_PAYLOAD_32_SIZE;
        context->elf.elf32.phdr[segment_index].p_filesz = new_segment_size;
        context->elf.elf32.phdr[segment_index].p_memsz  = new_segment_size;
        set_elf_segment_permission(context, segment_index, PF_W | PF_X | PF_R);
    }
...
static int update_section_values(t_woody_context* context, int section_index) {
    if (context->elf.is_64bit) {
        for (int i = section_index + 1;
             i < context->elf.elf64.ehdr->e_shnum - 1; i++)
            context->elf.elf64.shdr[i + 1].sh_offset
                = context->elf.elf64.shdr[i].sh_offset
                + context->elf.elf64.shdr[i].sh_size;

        int sh_count = context->elf.elf64.ehdr->e_shnum;
        context->elf.elf64.ehdr->e_shoff
            = context->elf.elf64.shdr[sh_count - 1].sh_offset
            + context->elf.elf64.shdr[sh_count - 1].sh_size;
    } else {
        for (int i = section_index + 1;
             i < context->elf.elf32.ehdr->e_shnum - 1; i++)
            context->elf.elf32.shdr[i + 1].sh_offset
                = context->elf.elf32.shdr[i].sh_offset
                + context->elf.elf32.shdr[i].sh_size;

        int sh_count = context->elf.elf32.ehdr->e_shnum;
        context->elf.elf32.ehdr->e_shoff
            = context->elf.elf32.shdr[sh_count - 1].sh_offset
            + context->elf.elf32.shdr[sh_count - 1].sh_size;
    }
...
static int update_text_permissions(t_woody_context* context) {
    ...
    set_elf_segment_permission(
        context, text_section_index, PF_W | PF_X | PF_R);

    return SUCCESS;
}

static int create_new_section(
    t_woody_context* context, int segment_index, int section_index) {
    ...
    if (context->elf.is_64bit) {
        ...
        updated_section_headers = realloc(context->elf.elf64.shdr,
        ...
        updated_section_data = realloc(context->elf.elf64.section_data,
        ...
        if (payload_section == NULL) {
            print_verbose(
                context, "Failed to initialize new section header\n");
            return ERR_INJECTION;
        }
        payload_section->sh_name
            = context->elf.elf64.shdr[context->elf.elf64.ehdr->e_shstrndx]
                  .sh_size;
        payload_section->sh_offset
            = context->elf.elf64.phdr[segment_index].p_offset
            + context->elf.elf64.phdr[segment_index].p_memsz;
        payload_section->sh_addr
            = context->elf.elf64.phdr[segment_index].p_vaddr
            + context->elf.elf64.phdr[segment_index].p_memsz;
        payload_section->sh_size = INJECTION_PAYLOAD_64_SIZE;

        // Calculate remaining size to move for section headers and section
        // data
        size_t remaining_size_to_move
            = (context->elf.elf64.ehdr->e_shnum - section_index - 1);
        ...
        memcpy(&context->elf.elf64.shdr[section_index], payload_section,
            sizeof(Elf64_Shdr));
        ...
        if (payload == NULL) {
            context->elf.elf64.section_data[section_index] = NULL;
            free(payload_section);
            free(section_data);
        ...
        // Resize section headers and section data
        updated_section_headers = realloc(context->elf.elf32.shdr,
        ...
        updated_section_data = realloc(context->elf.elf32.section_data,
        ...
        payload_section->sh_name
            = context->elf.elf32.shdr[context->elf.elf32.ehdr->e_shstrndx]
                  .sh_size;
        payload_section->sh_offset
            = context->elf.elf32.phdr[segment_index].p_offset
            + context->elf.elf32.phdr[segment_index].p_memsz;
        payload_section->sh_addr
            = context->elf.elf32.phdr[segment_index].p_vaddr
            + context->elf.elf32.phdr[segment_index].p_memsz;
        ...
        // data
        size_t remaining_size_to_move
            = (context->elf.elf32.ehdr->e_shnum - section_index - 1);
        ...
        // Insert the new section header
        memcpy(&context->elf.elf32.shdr[section_index], payload_section,
            sizeof(Elf32_Shdr));
...
int insert_new_section(t_woody_context* context) {
    ...
    int last_section_in_segment
        = find_last_section_in_segment(context, last_load_segment);
    if (last_section_in_segment == -1) {
        print_verbose(context, "Failed to find last section in segment\n");
        return ERR_INJECTION;
    }

    if (create_new_section(context, last_load_segment, last_section_in_segment)
        != SUCCESS) {
    ...
    if (update_section_symtab_sh_link_count(context) != SUCCESS) {
        print_verbose(
            context, "Failed to update section symtab sh_link count\n");
        return ERR_INJECTION;
    }
